        ...
        if jogos_dia:
            ...
            if piloto_automatico or st.button("🧠 Aplicar Poisson (Modo Manual)"):
                for j in jogos_dia: # Analisa TODOS os jogos encontrados na varredura
                    ...
                    # 2. NOVA CHAMADA: Recebe o MELHOR mercado da IA (BTTS, O25 ou O15)
                    mercado_ia, confianca_ia, odd_ia = analisar_jogo_matematicamente_real(media_c, media_f)
                    ...
                    if mercado_ia:
                        ...
                        # CHECA SE JÁ EXISTE NA PLANILHA (Trava de Duplicidade)
                        url_planilha = "https://docs.google.com/spreadsheets/d/1Y4D4t2svOeT24vnKcWnzDcwz7tPyRvkeDP8sSm_xPkQ/edit?usp=sharing"
                        df_check = conn.read(spreadsheet=url_planilha, ttl=0)
                        ja_existe = False
                        
                        if not df_check.empty:
                            # Verifica se já existe um registro com a mesma Casa e Previsão
                            ja_existe = any((df_check['Casa'] == casa) & (df_check['Previsao_IA'] == mercado_ia))

                        if not ja_existe and id_sinal not in st.session_state.sinais_enviados:
                            ...
                            # MOSTRAR O CARD (Garante que apareça na tela mesmo após dar F5)
                    ...
                    # Se a IA não atingiu os % mínimos, registra apenas no log do sistema
